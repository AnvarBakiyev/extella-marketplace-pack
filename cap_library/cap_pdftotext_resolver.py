# expert: cap_pdftotext_resolver
# description: Установка и проверка инструмента «pdftotext (PDF → текст)» (brew). Зови ПЕРЕД первым использованием этой способности.

def cap_pdftotext_resolver(confirm_install="no") -> str:
    import os, subprocess, json
    CANDS = ["/opt/homebrew/bin/pdftotext", "/usr/local/bin/pdftotext"]
    def rec(p):
        d = os.path.expanduser("~/.extella_cli"); os.makedirs(d, exist_ok=True)
        open(os.path.join(d, "pdftotext"), "w").write(p)
    def verify(p):
        try:
            r = subprocess.run([p] + ["-v"], capture_output=True, text=True, timeout=20)
            if r.returncode == 0:
    ...
    if not confirm_install or confirm_install.startswith("{{") or confirm_install.lower() != "yes":
        return json.dumps({"status":"missing","message":"pdftotext (PDF → текст) не установлен. confirm_install='yes' поставит через brew."}, ensure_ascii=False)
    ...
    try:
        subprocess.run([brew] + ["install", "poppler"], capture_output=True, text=True, timeout=280, env=env)
