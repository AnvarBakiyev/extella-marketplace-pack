# expert: cap_calibre_resolver
# description: Установка и проверка инструмента «Calibre (конвертация книг)» (brew). Зови ПЕРЕД первым использованием этой способности.

def cap_calibre_resolver(confirm_install="no") -> str:
    import os, subprocess, json
    CANDS = ["/opt/homebrew/bin/ebook-convert", "/Applications/calibre.app/Contents/MacOS/ebook-convert", "/usr/local/bin/ebook-convert"]
    def rec(p):
        d = os.path.expanduser("~/.extella_cli"); os.makedirs(d, exist_ok=True)
        open(os.path.join(d, "calibre"), "w").write(p)
    def verify(p):
        try:
    ...
    if not confirm_install or confirm_install.startswith("{{") or confirm_install.lower() != "yes":
        return json.dumps({"status":"missing","message":"Calibre (конвертация книг) не установлен. confirm_install='yes' поставит через brew."}, ensure_ascii=False)
    ...
    try:
        subprocess.run([brew] + ["install", "--cask", "calibre"], capture_output=True, text=True, timeout=900, env=env)
