# expert: cap_rsvg_to_png_batch
# description: SVG в картинку — Векторные SVG → обычные PNG — пачкой, локально Пакетно: операция «SVG → PNG» ко всем подходящим файлам в папке. Локально/офлайн.

def cap_rsvg_to_png_batch(in_dir="", out_dir="", _pad=None) -> str:
    ...
    pass
    srcs = sorted(glob.glob(os.path.join(in_dir, "*.svg")))
