# expert: cap_cwebp_resolver
# description: Установка и проверка инструмента «cwebp (в WebP)» (brew). Зови ПЕРЕД первым использованием этой способности.

def cap_cwebp_resolver(confirm_install="no") -> str:
    import os, subprocess, json
    CANDS = ["/opt/homebrew/bin/cwebp", "/usr/local/bin/cwebp"]
    def rec(p):
        d = os.path.expanduser("~/.extella_cli"); os.makedirs(d, exist_ok=True)
        open(os.path.join(d, "cwebp"), "w").write(p)
    def verify(p):
        try:
    ...
    if not confirm_install or confirm_install.startswith("{{") or confirm_install.lower() != "yes":
        return json.dumps({"status":"missing","message":"cwebp (в WebP) не установлен. confirm_install='yes' поставит через brew."}, ensure_ascii=False)
    ...
    try:
        subprocess.run([brew] + ["install", "webp"], capture_output=True, text=True, timeout=280, env=env)
