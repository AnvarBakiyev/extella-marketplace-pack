# expert: cap_exiftool_resolver
# description: Установка и проверка инструмента «ExifTool (метаданные фото)» (brew). Зови ПЕРЕД первым использованием этой способности.

def cap_exiftool_resolver(confirm_install="no") -> str:
    import os, subprocess, json
    CANDS = ["/opt/homebrew/bin/exiftool", "/usr/local/bin/exiftool"]
    def rec(p):
        d = os.path.expanduser("~/.extella_cli"); os.makedirs(d, exist_ok=True)
        open(os.path.join(d, "exiftool"), "w").write(p)
    def verify(p):
        try:
            r = subprocess.run([p] + ["-ver"], capture_output=True, text=True, timeout=20)
            if r.returncode == 0:
    ...
    if not confirm_install or confirm_install.startswith("{{") or confirm_install.lower() != "yes":
        return json.dumps({"status":"missing","message":"ExifTool (метаданные фото) не установлен. confirm_install='yes' поставит через brew."}, ensure_ascii=False)
    ...
    try:
        subprocess.run([brew] + ["install", "exiftool"], capture_output=True, text=True, timeout=280, env=env)
