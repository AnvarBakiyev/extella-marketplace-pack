# expert: cap_rsvg_to_png
# description: SVG в картинку — Векторные SVG → обычные PNG — пачкой, локально Операция «SVG → PNG», один файл. Локально/офлайн. Зови ЭТОТ эксперт, НЕ пиши shell-команду.

def cap_rsvg_to_png(input_path="", output_path="") -> str:
    import os, subprocess, json, shutil, tempfile

    def binpath():
        f = os.path.expanduser("~/.extella_cli/rsvg")
        if os.path.exists(f):
            p = open(f).read().strip()
    ...
    b = binpath()
    if not b:
        return json.dumps({"status":"error","message":"rsvg-convert (SVG → PNG) не установлен — сначала cap_rsvg_resolver(confirm_install='yes')"}, ensure_ascii=False)
    if not output_path or output_path.startswith("{{"):
        base, _ = os.path.splitext(input_path); output_path = base + ".png"
