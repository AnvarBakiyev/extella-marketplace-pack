# expert: cap_exiftool_strip
# description: Очистить метаданные фото — Убирает геолокацию и данные камеры из фото — приватность, локально Операция «Удалить метаданные», один файл. Локально/офлайн. Зови ЭТОТ эксперт, НЕ пиши shell-команду.

def cap_exiftool_strip(input_path="", output_path="") -> str:
    ...
    if not output_path or output_path.startswith("{{"):
        base, _ = os.path.splitext(input_path); output_path = base + "_clean.jpg"
