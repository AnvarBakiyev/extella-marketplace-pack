...
function tmpl(val,c){ if(typeof val!=='string'||!val.includes('{{')) return val;
  return val.replace(/\{\{([\s\S]*?)\}\}/g,(_,e)=>{ try{ const f=new Function('gpu','platform','arch','args','input','cwd','port','return ('+e+')');
    const r=f(c.gpu,c.platform,c.arch,c.args||{},c.input||{},c.cwd,c.port); return (r==null)?'':String(r);}catch(x){return '';} }); }
...
function evalWhen(when,c){ if(!when) return true;
  try{ const expr=String(when).replace(/^\{\{|\}\}$/g,''); return !!(new Function('gpu','platform','arch','args','return ('+expr+')')(c.gpu,c.platform,c.arch,c.args||{})); }catch(e){ return false; } }
...
async function resolve(root, entry, args, depth, out, kernel){
  ...
  const c={gpu:kernel.gpu,platform:kernel.platform,arch:kernel.arch,args:args||{},input:{event:['']},cwd:root,port:kernel._getPort()};
