def mcp_connect(server_id="", pkg_type="", pkg="", title="") -> str:
    ...
    def _abs(cmd0):
        try:
            ...
            return path_or_error(cmd0, repair=True)
        except Exception:
            return None, {"error_class":"client_runtime_missing","message":"Системный runtime Extella не установлен. Запустите Repair Extella Client."}
    ...
    exe, runtime = _abs(cmd[0])
    if not exe:
        return err(runtime.get("message") or (cmd[0] + " недоступен"))
    run_cmd = [exe] + cmd[1:]
    try:
