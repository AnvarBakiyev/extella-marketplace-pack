# expert: cap_graphviz_to_png
# description: Схемы из текста — Рисует диаграммы и графы из .dot-описания — локально Операция «Схема → PNG», один файл. Локально/офлайн. Зови ЭТОТ эксперт, НЕ пиши shell-команду.

def cap_graphviz_to_png(input_path="", output_path="") -> str:
    import os, subprocess, json, shutil, tempfile

    def binpath():
        f = os.path.expanduser("~/.extella_cli/graphviz")
        if os.path.exists(f):
            p = open(f).read().strip()
            if p and os.path.exists(p): return p
        p = shutil.which("dot")
        if p: return p
        for c in ["/opt/homebrew/bin/dot", "/usr/local/bin/dot"]:
