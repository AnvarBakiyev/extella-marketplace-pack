# expert: cap_qpdf_resolver
# description: Установка и проверка инструмента «qpdf (структура PDF)» (brew). Зови ПЕРЕД первым использованием этой способности.

def cap_qpdf_resolver(confirm_install="no") -> str:
    import os, subprocess, json
    CANDS = ["/opt/homebrew/bin/qpdf", "/usr/local/bin/qpdf"]
    def rec(p):
        d = os.path.expanduser("~/.extella_cli"); os.makedirs(d, exist_ok=True)
        open(os.path.join(d, "qpdf"), "w").write(p)
    def verify(p):
        try:
    ...
    if not confirm_install or confirm_install.startswith("{{") or confirm_install.lower() != "yes":
        return json.dumps({"status":"missing","message":"qpdf (структура PDF) не установлен. confirm_install='yes' поставит через brew."}, ensure_ascii=False)
    ...
    try:
        subprocess.run([brew] + ["install", "qpdf"], capture_output=True, text=True, timeout=280, env=env)
