# expert: cap_calibre_to_epub
# description: Конвертер книг — EPUB, MOBI, PDF, FB2 — конвертация электронных книг, локально Операция «В EPUB», один файл. Локально/офлайн. Зови ЭТОТ эксперт, НЕ пиши shell-команду.

def cap_calibre_to_epub(input_path="", output_path="") -> str:
    import os, subprocess, json, shutil, tempfile

    def binpath():
        f = os.path.expanduser("~/.extella_cli/calibre")
        if os.path.exists(f):
            p = open(f).read().strip()
            if p and os.path.exists(p): return p
        p = shutil.which("ebook-convert")
        if p: return p
        for c in ["/opt/homebrew/bin/ebook-convert", "/Applications/calibre.app/Contents/MacOS/ebook-convert", "/usr/local/bin/ebook-convert"]:
    ...
    b = binpath()
    if not b:
        return json.dumps({"status":"error","message":"Calibre (конвертация книг) не установлен — сначала cap_calibre_resolver(confirm_install='yes')"}, ensure_ascii=False)
    if not output_path or output_path.startswith("{{"):
        base, _ = os.path.splitext(input_path); output_path = base + ".epub"
