# expert: cap_oxipng_optimize_batch
# description: PNG без потерь — Ужимает PNG без потери качества (lossless) — пачкой, локально Пакетно: операция «Сжать PNG без потерь» ко всем подходящим файлам в папке. Локально/офлайн.

def cap_oxipng_optimize_batch(in_dir="", out_dir="", _pad=None) -> str:
    import os, subprocess, json, glob, shutil, tempfile

    def binpath():
        try:
            from extella_expert_bridge import path_or_error
            path, _state = path_or_error("oxipng", repair=False)
            return path
        except Exception:
            return None
    ...
    for src in srcs:
        ...
        dst = os.path.join(out_dir, stem + "_opt.png")
