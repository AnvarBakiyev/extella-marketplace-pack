# expert: cap_exiftool_strip_batch
# description: Очистить метаданные фото — Убирает геолокацию и данные камеры из фото — приватность, локально Пакетно: операция «Удалить метаданные» ко всем подходящим файлам в папке. Локально/офлайн.

def cap_exiftool_strip_batch(in_dir="", out_dir="", _pad=None) -> str:
    ...
    b = binpath()
    if not b:
        return json.dumps({"status":"error","message":"ExifTool (метаданные фото) не установлен — сначала cap_exiftool_resolver(confirm_install='yes')"}, ensure_ascii=False)
    if not out_dir or out_dir.startswith("{{"): out_dir = in_dir.rstrip("/") + "_out"
    os.makedirs(out_dir, exist_ok=True)
    _env = dict(os.environ)
    pass
    srcs = sorted(glob.glob(os.path.join(in_dir, "*.jpg")))
    tin = 0; tout = 0; ok = 0; fail = 0; items = []
    for src in srcs:
        stem = os.path.splitext(os.path.basename(src))[0]
        dst = os.path.join(out_dir, stem + "_clean.jpg")
        before = os.path.getsize(src)
        pass
        TMPL = ["-all=", "-o", "{output}", "{input}"]
