# expert: cap_gifsicle_optimize_batch
# description: Сжать GIF — Оптимизирует и уменьшает GIF-анимации — пачкой, локально Пакетно: операция «Оптимизировать GIF» ко всем подходящим файлам в папке. Локально/офлайн.

def cap_gifsicle_optimize_batch(in_dir="", out_dir="", _pad=None) -> str:
    import os, subprocess, json, glob, shutil, tempfile

    def binpath():
        f = os.path.expanduser("~/.extella_cli/gifsicle")
        if os.path.exists(f):
            p = open(f).read().strip()
            if p and os.path.exists(p): return p
        p = shutil.which("gifsicle")
        if p: return p
        for c in ["/opt/homebrew/bin/gifsicle", "/usr/local/bin/gifsicle"]:
    ...
    pass
    srcs = sorted(glob.glob(os.path.join(in_dir, "*.gif")))
    tin = 0; tout = 0; ok = 0; fail = 0; items = []
    for src in srcs:
        stem = os.path.splitext(os.path.basename(src))[0]
        dst = os.path.join(out_dir, stem + "_opt.gif")
