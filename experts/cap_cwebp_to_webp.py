# expert: cap_cwebp_to_webp
# description: В WebP — Картинки в лёгкий формат WebP для сайтов — пачкой, локально Операция «В WebP», один файл. Локально/офлайн. Зови ЭТОТ эксперт, НЕ пиши shell-команду.

def cap_cwebp_to_webp(input_path="", output_path="", quality="80") -> str:
    ...
    b = binpath()
    if not b:
        return json.dumps({"status":"error","message":"cwebp (в WebP) не установлен — сначала cap_cwebp_resolver(confirm_install='yes')"}, ensure_ascii=False)
    if not output_path or output_path.startswith("{{"):
        base, _ = os.path.splitext(input_path); output_path = base + ".webp"
    os.makedirs(os.path.dirname(os.path.abspath(output_path)) or ".", exist_ok=True)
    before = os.path.getsize(input_path)
    TMPL = ["{input}", "-q", "{quality}", "-o", "{output}"]
