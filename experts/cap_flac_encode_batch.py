# expert: cap_flac_encode_batch
# description: Сжать аудио (FLAC) — WAV в FLAC без потери качества — пачкой, локально Пакетно: операция «WAV → FLAC» ко всем подходящим файлам в папке. Локально/офлайн.

def cap_flac_encode_batch(in_dir="", out_dir="", _pad=None) -> str:
    import os, subprocess, json, glob, shutil, tempfile

    def binpath():
        f = os.path.expanduser("~/.extella_cli/flac")
        if os.path.exists(f):
            p = open(f).read().strip()
            if p and os.path.exists(p): return p
        p = shutil.which("flac")
        if p: return p
        for c in ["/opt/homebrew/bin/flac", "/usr/local/bin/flac"]:
    ...
    pass
    srcs = sorted(glob.glob(os.path.join(in_dir, "*.wav")))
