# expert: cap_pandoc_md_to_html
# description: Документы из Markdown — Собрать Word/HTML из шаблона и данных — локально, пачкой Операция «Markdown → HTML», один файл. Локально/офлайн. Зови ЭТОТ эксперт, НЕ пиши shell-команду.

def cap_pandoc_md_to_html(input_path="", output_path="") -> str:
    import os, subprocess, json, shutil, tempfile

    def binpath():
        try:
            from extella_expert_bridge import path_or_error
            path, _state = path_or_error("pandoc", repair=False)
            return path
        except Exception:
            return None
    ...
    if not output_path or output_path.startswith("{{"):
        base, _ = os.path.splitext(input_path); output_path = base + ".html"
    os.makedirs(os.path.dirname(os.path.abspath(output_path)) or ".", exist_ok=True)
    before = os.path.getsize(input_path)
    TMPL = ["{input}", "-s", "-o", "{output}"]
