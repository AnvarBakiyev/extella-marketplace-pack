# expert: cap_libreoffice_resolver
# description: Установка и проверка инструмента «LibreOffice (Office → PDF)» (brew). Зови ПЕРЕД первым использованием этой способности.

def cap_libreoffice_resolver(confirm_install="no") -> str:
    ...
    def rec(p):
        ...
        open(os.path.join(d, "libreoffice"), "w").write(p)
    def verify(p):
        try:
    ...
    if not confirm_install or confirm_install.startswith("{{") or confirm_install.lower() != "yes":
        return json.dumps({"status":"missing","message":"LibreOffice (Office → PDF) не установлен. confirm_install='yes' поставит через brew."}, ensure_ascii=False)
    ...
    try:
        subprocess.run([brew] + ["install", "--cask", "libreoffice"], capture_output=True, text=True, timeout=900, env=env)
