# expert: cap_ffmpeg_to_mp4_batch
# description: Видео и аудио — Перекодировать, сжать и извлечь аудио из медиатеки Пакетно: операция «В MP4 (H.264)» ко всем подходящим файлам в папке. Локально/офлайн.

def cap_ffmpeg_to_mp4_batch(in_dir="", out_dir="", _pad=None) -> str:
    import os, subprocess, json, glob, shutil, tempfile

    def binpath():
        f = os.path.expanduser("~/.extella_cli/ffmpeg")
        if os.path.exists(f):
            p = open(f).read().strip()
    ...
    pass
    srcs = sorted(glob.glob(os.path.join(in_dir, "*.mov")))
    tin = 0; tout = 0; ok = 0; fail = 0; items = []
    for src in srcs:
        stem = os.path.splitext(os.path.basename(src))[0]
        dst = os.path.join(out_dir, stem + "_h264.mp4")
        before = os.path.getsize(src)
        pass
        TMPL = ["-y", "-i", "{input}", "-c:v", "libx264", "-preset", "fast", "-c:a", "aac", "{output}"]
