# expert: cap_qpdf_optimize
# description: Повернуть и оптимизировать PDF — Поворот и веб-оптимизация PDF без потери качества Операция «Оптимизировать для веба», один файл. Локально/офлайн. Зови ЭТОТ эксперт, НЕ пиши shell-команду.

def cap_qpdf_optimize(input_path="", output_path="") -> str:
    ...
    if not output_path or output_path.startswith("{{"):
        base, _ = os.path.splitext(input_path); output_path = base + "_web.pdf"
    os.makedirs(os.path.dirname(os.path.abspath(output_path)) or ".", exist_ok=True)
    before = os.path.getsize(input_path)
    TMPL = ["--linearize", "{input}", "{output}"]
