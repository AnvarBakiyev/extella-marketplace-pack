# expert: cap_pdftotext_to_txt_batch
# description: Текст из PDF — Вытаскивает чистый текст из PDF — пачкой, локально Пакетно: операция «PDF → текст» ко всем подходящим файлам в папке. Локально/офлайн.

def cap_pdftotext_to_txt_batch(in_dir="", out_dir="", _pad=None) -> str:
    import os, subprocess, json, glob, shutil, tempfile

    def binpath():
        try:
            from extella_expert_bridge import path_or_error
            path, _state = path_or_error("pdftotext", repair=False)
            return path
        except Exception:
            return None
