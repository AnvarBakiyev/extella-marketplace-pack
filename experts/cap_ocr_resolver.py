...
def cap_ocr_resolver(confirm_install="no") -> str:
    ...
    TESS = {"dirs": ["/opt/homebrew/share/tessdata", "/usr/local/share/tessdata"], "files": {"rus": "https://github.com/tesseract-ocr/tessdata_fast/raw/main/rus.traineddata"}}
    def rec(p):
        d = os.path.expanduser("~/.extella_cli"); os.makedirs(d, exist_ok=True)
        open(os.path.join(d, "ocr"), "w").write(p)
    def aug():
    ...
    if not confirm_install or confirm_install.startswith("{{") or confirm_install.lower() != "yes":
        return json.dumps({"status": "missing", "message": "OCR (поиск по сканам) не установлен. confirm_install='yes' поставит все части."}, ensure_ascii=False)
