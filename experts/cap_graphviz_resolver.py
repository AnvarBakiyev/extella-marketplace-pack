# expert: cap_graphviz_resolver
# description: Установка и проверка инструмента «Graphviz (схемы из текста)» (brew). Зови ПЕРЕД первым использованием этой способности.

def cap_graphviz_resolver(confirm_install="no") -> str:
    import os, subprocess, json
    CANDS = ["/opt/homebrew/bin/dot", "/usr/local/bin/dot"]
    def rec(p):
        d = os.path.expanduser("~/.extella_cli"); os.makedirs(d, exist_ok=True)
        open(os.path.join(d, "graphviz"), "w").write(p)
    def verify(p):
        try:
    ...
    if not confirm_install or confirm_install.startswith("{{") or confirm_install.lower() != "yes":
        return json.dumps({"status":"missing","message":"Graphviz (схемы из текста) не установлен. confirm_install='yes' поставит через brew."}, ensure_ascii=False)
    ...
    try:
        subprocess.run([brew] + ["install", "graphviz"], capture_output=True, text=True, timeout=280, env=env)
