# expert: cap_ocr_searchable_batch
# description: Поиск по сканам (OCR) — Сканы и фото-PDF → документы с полнотекстовым поиском Пакетно: операция «Скан → PDF с поиском» ко всем подходящим файлам в папке. Локально/офлайн.

def cap_ocr_searchable_batch(in_dir="", out_dir="", lang="rus+eng") -> str:
    ...
    for src in srcs:
        ...
        dst = os.path.join(out_dir, stem + "_ocr.pdf")
        before = os.path.getsize(src)
        pass
        TMPL = ["-l", "{lang}", "--skip-text", "--output-type", "pdf", "{input}", "{output}"]
        SUB = {"input": src, "output": dst, "lang": lang}
        argv = [b]
        for tok in TMPL:
