...
# description: Установка и проверка инструмента «ffmpeg (видео и аудио)» (brew). Зови ПЕРЕД первым использованием этой способности.
...
def cap_ffmpeg_resolver(confirm_install="no") -> str:
    ...
    def rec(p):
        ...
        open(os.path.join(d, "ffmpeg"), "w").write(p)
    def verify(p):
        try:
    ...
    if not confirm_install or confirm_install.startswith("{{") or confirm_install.lower() != "yes":
        return json.dumps({"status":"missing","message":"ffmpeg (видео и аудио) не установлен. confirm_install='yes' поставит через brew."}, ensure_ascii=False)
    ...
    try:
        subprocess.run([brew] + ["install", "ffmpeg"], capture_output=True, text=True, timeout=280, env=env)
