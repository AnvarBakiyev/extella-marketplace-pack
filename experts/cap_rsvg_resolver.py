# expert: cap_rsvg_resolver
# description: Установка и проверка инструмента «rsvg-convert (SVG → PNG)» (brew). Зови ПЕРЕД первым использованием этой способности.

def cap_rsvg_resolver(confirm_install="no") -> str:
    import os, subprocess, json
    CANDS = ["/opt/homebrew/bin/rsvg-convert", "/usr/local/bin/rsvg-convert"]
    def rec(p):
        d = os.path.expanduser("~/.extella_cli"); os.makedirs(d, exist_ok=True)
        open(os.path.join(d, "rsvg"), "w").write(p)
    def verify(p):
        try:
    ...
    if not confirm_install or confirm_install.startswith("{{") or confirm_install.lower() != "yes":
        return json.dumps({"status":"missing","message":"rsvg-convert (SVG → PNG) не установлен. confirm_install='yes' поставит через brew."}, ensure_ascii=False)
    ...
    try:
        subprocess.run([brew] + ["install", "librsvg"], capture_output=True, text=True, timeout=280, env=env)
