# expert: cap_pandoc_md_to_html_batch
# description: Документы из Markdown — Собрать Word/HTML из шаблона и данных — локально, пачкой Пакетно: операция «Markdown → HTML» ко всем подходящим файлам в папке. Локально/офлайн.

def cap_pandoc_md_to_html_batch(in_dir="", out_dir="", _pad=None) -> str:
    ...
    pass
    srcs = sorted(glob.glob(os.path.join(in_dir, "*.md")))
