# expert: cap_qpdf_rotate_batch
# description: Повернуть и оптимизировать PDF — Поворот и веб-оптимизация PDF без потери качества Пакетно: операция «Повернуть» ко всем подходящим файлам в папке. Локально/офлайн.

def cap_qpdf_rotate_batch(in_dir="", out_dir="", angle="+90") -> str:
    ...
    for src in srcs:
        ...
        dst = os.path.join(out_dir, stem + "_rotated.pdf")
        before = os.path.getsize(src)
        pass
        TMPL = ["--rotate={angle}", "{input}", "{output}"]
        SUB = {"input": src, "output": dst, "angle": angle}
        argv = [b]
        for tok in TMPL:
