# expert: cap_img2pdf_to_pdf
# description: Картинки → PDF — Собрать PDF из картинок и сканов — локально Операция «Картинка → PDF», один файл. Локально/офлайн. Зови ЭТОТ эксперт, НЕ пиши shell-команду.

def cap_img2pdf_to_pdf(input_path="", output_path="") -> str:
    ...
    b = binpath()
    if not b:
        return json.dumps({"status":"error","message":"img2pdf (картинки → PDF) не установлен — сначала cap_img2pdf_resolver(confirm_install='yes')"}, ensure_ascii=False)
